...
// music data is included as .s file generated by the text2data tool from
// music.ftm
...
#define SCREEN_HACKER (LEVELS_ALL + 2)
...
#define MUSIC_LOSE 5
#define MUSIC_HACKER 3

// palettes data
// all the palettes are designed in NES Screen Tool, then copy/pasted here
// with Palettes/Put C data to clipboard function

const unsigned char palTitle[16] = {0x0f, 0x0f, 0x0f, 0x0f, 0x0f, 0x1c,
                                    0x2c, 0x3c, 0x0f, 0x12, 0x22, 0x32,
                                    0x0f, 0x14, 0x24, 0x34};

const unsigned char palGame1[16] = {0x0f, 0x11, 0x32, 0x30, 0x0f, 0x1c,
                                    0x2c, 0x3c, 0x0f, 0x09, 0x27, 0x38,
                                    0x0f, 0x11, 0x21, 0x31};

const unsigned char palGame2[16] = {0x0f, 0x11, 0x32, 0x30, 0x0f, 0x11,
                                    0x21, 0x31, 0x0f, 0x07, 0x27, 0x38,
                                    0x0f, 0x13, 0x23, 0x33};

const unsigned char palGame3[16] = {0x0f, 0x11, 0x32, 0x30, 0x0f, 0x15,
                                    0x25, 0x35, 0x0f, 0x05, 0x27, 0x38,
                                    0x0f, 0x13, 0x23, 0x33};

const unsigned char palGame4[16] = {0x0f, 0x11, 0x32, 0x30, 0x0f, 0x19,
                                    0x29, 0x39, 0x0f, 0x0b, 0x27, 0x38,
                                    0x0f, 0x17, 0x27, 0x37};

const unsigned char palGame5[16] = {0x0f, 0x11, 0x32, 0x30, 0x0f, 0x16,
                                    0x26, 0x36, 0x0f, 0x07, 0x27, 0x38,
                                    0x0f, 0x18, 0x28, 0x38};

const unsigned char palGameSpr[16] = {0x0f, 0x0f, 0x29, 0x30, 0x0f, 0x0f,
                                      0x26, 0x30, 0x0f, 0x0f, 0x24, 0x30,
                                      0x0f, 0x0f, 0x21, 0x30};

// metasprites

const unsigned char sprPlayer[] = {0, -1,   0x49, 0, 8, -1,   0x4a, 0,  0,
                                   7, 0x4b, 0,    8, 7, 0x4c, 0,    128};

const unsigned char sprEnemy1[] = {0, -1,   0x4d, 1, 8, -1,   0x4e, 1,  0,
                                   7, 0x4f, 1,    8, 7, 0x50, 1,    128};

const unsigned char sprEnemy2[] = {0, -1,   0x4d, 2, 8, -1,   0x4e, 2,  0,
                                   7, 0x4f, 2,    8, 7, 0x50, 2,    128};

const unsigned char sprEnemy3[] = {0, -1,   0x4d, 3, 8, -1,   0x4e, 3,  0,
                                   7, 0x4f, 3,    8, 7, 0x50, 3,    128};

// list of metasprites

const unsigned char *const sprListPlayer[] = {sprPlayer, sprEnemy1, sprEnemy2,
                                              sprEnemy3};
...
const unsigned char *const levelList[LEVELS_ALL * 2] = {
    level1_nam, palGame1, level2_nam, palGame2, level3_nam, palGame3,
    level4_nam, palGame3, level5_nam, palGame3, level6_nam, palGame1};
static unsigned char win[LEVELS_ALL] = {0};
...
const unsigned char updateListData[] = {
    ...
    0x28, 0x00, TILE_EMPTY, 0x28, 0x00, TILE_EMPTY, 0x20, 0x4f,
    0x10,             // these three entires are used to
                      // display
    0x20, 0x50, 0x10, // number of the collected items
    0x20, 0x51, 0x10, NT_UPD_EOF};

// a nametable string with the game stats, created in NES Screen Tool
// and copy/pasted here with Shift+C

const unsigned char statsStr[27] = {0x2c, 0x25, 0x36, 0x25, 0x2c, 0x1a, 0x00,
                                    0x00, 0x27, 0x25, 0x2d, 0x33, 0x1a, 0x00,
                                    0x00, 0x00, 0x0f, 0x00, 0x00, 0x00, 0x00,
                                    0x2c, 0x29, 0x36, 0x25, 0x33, 0x1a};

// list of screens such as level number, game over, and well done
// contains pointers to the packed nametables

const unsigned char *screenList[4] = {level_nam, gameover_nam, welldone_nam,
                                      hacker_nam};

// list of sub songs for corresponding screens

const unsigned char screenMusicList[4] = {MUSIC_LEVEL, MUSIC_GAME_OVER,
                                          MUSIC_WELL_DONE, MUSIC_HACKER};
...
const unsigned char largeNums[12 * 3] = {
    0x7a, 0x7b, 0x7c, 0x7d, 0x7c, 0x7d, 0x7e, 0x7f, 0x80, 0x81, 0x9a, 0x9b,
    0x86, 0x7b, 0x87, 0x88, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x7d, 0x80, 0x7d,
    0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x7f, 0x94, 0x95, 0x9c, 0x95};

// array for game map, contains walls, empty spaces, and items

static unsigned char map[MAP_WDT * MAP_HGT];

// put all the subsequent global vars into zeropage, to make code faster and
// shorter
...
void title_screen(void) {
    scroll(-8, 240); // title is aligned to the color attributes, so shift it a
                     // bit to the right

    vram_adr(NAMETABLE_A);
    vram_unrle(title_nam);

    vram_adr(NAMETABLE_C); // clear second nametable, as it is visible in the
                           // jumping effect
    vram_fill(0, 1024);
    ...
    while (1) {
        ...
        if (wait) {
            ...
            pal_col(2, (frame_cnt & 32) ? 0x0f
                                        : 0x20); // blinking press start text
            ++frame_cnt;
        }
    }
...
void show_screen(unsigned char num) {
    scroll(-4,
           0); // all the screens are misaligneg horizontally by half of a tile
    ...
    i16 = (num == SCREEN_GAMEOVER || num == SCREEN_HACKER)
              ? 0x2535
              : 0x2131; // two colors for flashing text in LSB and MSB
...
void game_loop(void) {
    ...
    if (game_level >= LEVELS_WIN) {
        hacker = TRUE;
        show_screen(SCREEN_HACKER);
        return;
    }
    ...
    for (i = 2; i < MAP_HGT + 2; ++i) {
        ...
        for (j = 0; j < MAP_WDT << 1; j += 2) {
            ...
            case TILE_ENEMY3:
                ...
                player_speed[player_all] =
                    (spr == TILE_PLAYER) ? 2 << FP_BITS
                                         : 10 + ((spr - TILE_ENEMY1) << 1);
                ++player_all;
                wait += 16;
                spr = TILE_EMPTY;
                break;
    ...
    while (!game_done) {
        ...
        // right after object parameters were changed, so either OAM update
        // should be in a function that called before the loop and at the end of
        // the loop, or wait for NMI should be placed there otherwise you would
        // have situation update-wait-display, i.e. one frame delay between
        // action and display of its result
        ...
        // CHR bank switching animation with different speed for background and
        // sprites

        bank_bg((frame_cnt >> 4) & 1);
        bank_spr((frame_cnt >> 3) & 1);

        // a counter that does not allow objects to move while spawn animation
        // plays
        ...
        if (wait) {
            ...
            if (!wait)
                music_play(
                    MUSIC_GAME); // start the music when all the objects spawned
        }
        ...
        for (i = 0; i < player_all; ++i) {
            ...
            if (wait)
                continue; // don't process object movements if spawn animation
                          // is running

            // check collision of an enemy object with player object
            // NOT logic is used here, check
            // http://gendev.spritesmind.net/page-collide.html

            if (i) {
                if (!((player_x[i] + (4 << FP_BITS)) >=
                          (player_x[0] + (12 << FP_BITS)) ||
                      (player_x[i] + (12 << FP_BITS)) <
                          (player_x[0] + (4 << FP_BITS)) ||
                      (player_y[i] + (4 << FP_BITS)) >=
                          (player_y[0] + (12 << FP_BITS)) ||
                      (player_y[i] + (12 << FP_BITS)) <
                          (player_y[0] + (4 << FP_BITS)))) {
            ...
            if (player_cnt[i]) {
                ...
                // if move from one tile to another is over, realign the object
                // to tile grid it is needed because when it moves with
                // non-integer speed, it could overrun the destination tile a
                // little bit, and thus can't take a turn properly
                ...
                if (player_cnt[i] <= 0) {
                    if (player_cnt[i] < 0) // overrun
                    {
                        ...
                        // 0xff is a coordinate mask that leaves only integer
                        // tile offeset it is 8:4:4 here, where 8 is integer
                        // tile coordinate, first 4 is offset in the tile, which
                        // is 16 pixels wide, and second 4 is fixed point
                        // resolution

                        player_x[i] = (player_x[i] & 0xff00) +
                                      (player_dir[i] == DIR_LEFT ? 0x100 : 0);
                        player_y[i] = (player_y[i] & 0xff00) +
                                      (player_dir[i] == DIR_UP ? 0x100 : 0);
                    }

                    // it is is the player object, check if there is an item in
                    // the new tile
                    if (!i) {
                        i16 =
                            MAP_ADR((player_x[i] >> (TILE_SIZE_BIT + FP_BITS)),
                                    (player_y[i] >> (TILE_SIZE_BIT + FP_BITS)));

                        if (map[i16] == TILE_ITEM) {
                            map[i16] =
                                TILE_EMPTY; // mark as collected in the game map
                            ...
                            i16 = NAMETABLE_A + 0x0080 +
                                      (((player_y[i] >>
                                         (TILE_SIZE_BIT + FP_BITS)) -
                                        2)
                                       << 6) |
                                  ((player_x[i] >> (TILE_SIZE_BIT + FP_BITS))
                                   << 1);

                            // replace it with empty tile through the update
                            // list
                            ...
                            // update number of collected items in the game
                            // stats
            ...
            if (!player_cnt[i]) // movement to the next tile is done, set up new
                                // movement
            {
                if (!i) // this is the player, process controls
                {
                    // get gamepad state, it was previously polled with
                    // pad_trigger
                    ...
                    if (j & player_dir[0]) {
                        j &= ~player_dir[0]; // remove the direction from
                                             // further check
                ...
                } else // this is an enemy, run AI
                {
                    ...
                    // first we create list of all directions that are possible
                    // to take excluding the direction that is opposite to
                    // previous one
...
void main(void) {
    while (1) // infinite loop, title-gameplay
    {
        ...
        game_lives = 4;
        hacker = FALSE;
        for (i = 0; i < LEVELS_WIN; ++i)
            win[i] = FALSE;

        while (!hacker && game_lives &&
               game_level < LEVELS_WIN) // loop for gameplay
        {
        ...
        if (!game_lives)
            ...
            if (!hacker)
                show_screen(SCREEN_WELLDONE); // show game results
        }
    }
}
